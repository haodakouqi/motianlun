
export interface Colors {
  name: string // 国际化版本请提供国际化字符串如：message.system.setting.style.default，非国际化版本直接写字符串，如：默认菜单风格
  elementPlus: any,
  menu: any,
  header: {
    background: string
    textColor: string
    itemHoverColor: string
    breadcrumbTextColor: string
    borderColor: string
    tabBackground: string
  }
  container: {
    background: string
    mainBackground: string
  }
  page: {
    background: string
    color: string
    tipColor: string
    borderColor: string
  }
}

export interface themeStyle {
  default: Colors
  light: Colors
  dark: Colors,
  [propName: string]: Colors
}
export const themeStyle: themeStyle = {
  'default': {
    name: 'message.system.setting.style.default',
    elementPlus: {
      "--el-color-primary":"#4d70ff"
    },
    menu: {
      "--el-menu-text-color": '#fff',
      "--el-menu-active-color": '#fff',
      "--el-menu-bg-color": 'rgb(25, 26, 35)',
      "--el-menu-hover-bg-color": 'rgb(25, 26, 35)',
    },
    header: {
      background: '#fff',
      textColor: '#303133',
      itemHoverColor: 'rgba(0,0,0,.06)',
      breadcrumbTextColor: '#606266',
      borderColor: '#d8dce5',
      tabBackground: '#fff'
    },
    container: {
      background: '#f0f2f5',
      mainBackground: '#fff'
    },
    page: {
      background: '#fff',
      color: '#303133',
      tipColor: 'rgba(0, 0, 0, 0.45)',
      borderColor: '#ebeef5'
    }
  },
  'light': {
    name: 'message.system.setting.style.light',
    elementPlus: {
      "--el-color-primary":"#d60f20"
    },
    menu: {
      "--el-menu-text-color": '#272727',
      "--el-menu-active-color": '#272727',
      "--el-menu-bg-color": '#fff',
      "--el-menu-hover-bg-color": '#fff',
    },
    header: {
      background: '#fff',
      textColor: '#303133',
      itemHoverColor: 'rgba(0,0,0,.025)',
      breadcrumbTextColor: '#606266',
      borderColor: '#d8dce5',
      tabBackground: '#fff'
    },
    container: {
      background: '#f0f2f5',
      mainBackground: '#fff'
    },
    page: {
      background: '#fff',
      color: '#303133',
      tipColor: 'rgba(0, 0, 0, 0.45)',
      borderColor: '#ebeef5'
    }
  },
  'chinese': {
    name: 'message.system.setting.style.chinese',
    elementPlus: {
      "--el-color-primary":"#ac25e6"
    },
    menu: {
      "--el-menu-text-color": '#c7c7c7',
      "--el-menu-active-color": '#c7c7c7',
      "--el-menu-bg-color": '#232323',
      "--el-menu-hover-bg-color": '#232323',
    },
    header: {
      background: '#f1f0ed',
      textColor: '#303133',
      itemHoverColor: 'rgba(0,0,0,.025)',
      breadcrumbTextColor: '#606266',
      borderColor: '#d8dce5',
      tabBackground: 'rgba(216, 216, 216, 0.51)'
    },
    container: {
      background: 'rgba(255, 255, 255, 0.92)',
      mainBackground: 'rgba(255, 255, 255, 0.92)'
    },
    page: {
      background: 'rgba(255, 255, 255, 0.92)',
      color: '#303133',
      tipColor: 'rgba(0, 0, 0, 0.45)',
      borderColor: '#ebeef5'
    }
  },
  'dark': {
    name: 'message.system.setting.style.dark',
    elementPlus: {
      "--el-color-primary":"#13c2c2"
    },
    menu: {
      "--el-menu-text-color": '#bbb',
      "--el-menu-active-color": '#bbb',
      "--el-menu-bg-color": '#18181c',
      "--el-menu-hover-bg-color": '#18181c',
    },
    header: {
      background: '#18181c',
      textColor: '#e3e3e4',
      itemHoverColor: '#000',
      breadcrumbTextColor: '#fff',
      borderColor: '#3e3e3e',
      tabBackground: '#1b1b1b'
    },
    container: {
      background: '#000',
      mainBackground: '#18181c'
    },
    page: {
      background: '#18181c',
      color: '#c7c7c7',
      tipColor: 'rgba(255, 255, 255, 0.45)',
      borderColor: '#3e3e3e'
    }
  }
}