...
import { listCustomerRebate, deleteCustomerRebate } from "@/api/custom";
import {
  Delete,
  Edit,
...
function usetable() {
    ...
    const getList = async() => {
      try {
        ...
        const res = await listCustomerRebate(params);
    ...
    ElMessageBox.confirm("确定删除该月返点吗？", "提示", {
      ...
      beforeClose: async(action, ctx, close) => {
        try {
          ...
          await deleteCustomerRebate(row.id);
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const openDialog = () => {
      const { platformList, customerCode } = props.dialogData;
